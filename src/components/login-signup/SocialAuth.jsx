...
import Divider from "../login-signup/Divider";
  ...
  return (
    <div className="social-auth text-center">
      {google && (
        <Link
          to="#"
          className="mt-4 flex items-center justify-center mx-5 py-3 gap-2 bg-white text-gray-700 border border-gray-300 rounded-3xl font-medium shadow-[inset_0_1px_2px_rgba(0,0,0,0.12)] hover:shadow-[inset_0_1px_2px_rgba(0,0,0,0.12)] hover:bg-blue-200 focus:outline-none active:ring-1 active:ring-blue-600 active:ring-offset-1 active:scale-[0.99] transition-all ease-out duration-200"
        >
          {" "}
          <svg
      ...
      {text && textLink ? (
        <span className="block my-4">
          {text} <Link to="/login" className="font-semibold focus:outline-none text-blue-600 hover:underline transition-all duration-200 ease-out py-2">{textLink}</Link>
        </span>
      ) : (
        textLink && (
          <span className="block my-4">
            <Link
              to="/login"
              className="font-semibold active:ring-1 focus:ring-blue-600 focus:outline-none text-blue-600 hover:underline hover:bg-blue-100 transition-all duration-200 ease-out rounded-3xl px-4 py-2"
            >
              {textLink}
            </Link>
          </span>
