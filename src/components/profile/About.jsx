...
const About = ({ profileUser, setProfileUser }) => {
  ...
  const aboutText =
    profileUser?.about ||
    (isOwner ? "write something about yourself." : "No information provided.");
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (el) {
        ...
        setShowBtn(isOverflowing || expanded);
      }
    }, 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log("Update about response: ", data);
      if (res.ok) {
        setIsEditing(false);
        await fetchUser();
        setProfileUser(data);
  ...
  return (
    <div className="bg-white rounded-xl px-6 pt-6 pb-7 shadow-sm overflow-hidden hover:shadow-lg hover:shadow-gray-300 shadow-gray-300 transition-shadow duration-200">
      <div className="flex justify-between items-center">
        ...
        {isOwner && (
          <Link
            onClick={() => {
              setIsEditing(true);
              setAboutInput(profileUser?.about || "");
            }}
            to="#"
            className="bg-white hover:bg-[#F3F3F3] w-10 h-10 rounded-full p-1 flex justify-center items-center transition-all ease-in-out duration-200"
          >
            <svg
              width="25px"
              height="25px"
        ...
        {showBtn && (
          <div className="flex justify-end">
            <button
              ...
              className="text-blue-500 hover:text-blue-700 text-sm font-medium mt-2 hover:underline transition-all duration-200 ease cursor-pointer"
            >
      ...
      <div className="border-2 border-[#EEEEEE] p-3 rounded-md flex items-center justify-between">
        ...
        <Link
          to="#"
          className="hover:bg-[#F3F3F3] rounded-full h-8 w-8 flex items-center justify-center transition-all ease-in-out duration-200"
        >
          <svg
            width="20px"
            height="20px"
      ...
      {isEditing && (
        ...
        >
          <div
            className="bg-white w-[800px] rounded-xl p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-semibold mb-3">Edit About</h2>

            <textarea
              value={aboutInput}
              onChange={(e) => setAboutInput(e.target.value)}
              className="w-full border border-gray-400 rounded-md p-2 h-32 outline-none"
            />

            <div className="flex justify-end gap-3 mt-4">
              <BlackBtn onClick={() => setIsEditing(false)} text="Cancel" />
              <BlueBtn
                onClick={handleSave}
