...
const ProfileHeader = ({ profileUser }) => {
  ...
  const isOwner = String(user?._id) === String(profileUser?._id);

  if (!profileUser) return <p>Loading...</p>;

  console.log("Logged user ID:", user?._id);
  console.log("Profile user ID:", profileUser?._id);
  console.log("Is Owner:", String(user?._id) === String(profileUser?._id));
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-lg hover:shadow-gray-300 shadow-gray-300 transition-shadow duration-200">
      <div className="w-full relative h-40 md:h-50 rounded-xl">
        <img
          src={profileUser?.coverImage || cover}
          alt="Cover Image"
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:gap-10">
        <div className="md:my-3 my-1 px-6">
          ...
          <p className="text-sm text-gray-500 my-2">
            {profileUser?.location || "Indore, Madhya Pradesh, India ."}
            <Link
              to="#"
          ...
          <div className="flex md:gap-3 gap-2 mb-6 flex-wrap">
            {isOwner ? (
              <>
                ...
                <BlueBtn text="Add profile section" />
                <BlueBtn text="Add custom button" />
                <BlackBtn text="Resources" />
              </>
            ) : (
              <>
                <BlueBtn text="Connect" />
                <BlackBtn text="Message" />
              </>
            )}
          </div>
